use crate::errors::MyError;  // crate 用的是绝对路径
...
pub async fn get_courses_for_teacher_db(pool: &PgPool, teacher_id: i32) -> Result<Vec<Course>, MyError> {
    let rows: Vec<Course> = sqlx::query_as!( // 因为course 实现了FromRow这个trait，所以使用query_as
        Course,
        r#"SELECT * FROM course WHERE teacher_id = $1"#,
        teacher_id
    )
        .fetch_all(pool)
        .await?; // 出现错误将错误传到handler去
    Ok(rows)
}
...
pub async fn get_course_details_db(
    pool: &PgPool,
    teacher_id: i32,
    course_id: i32) -> Result<Course, MyError> {
...
pub async fn post_new_course_db(
    pool: &PgPool,
    new_course: CreateCourse) -> Result<Course, MyError> {
    let row = sqlx::query_as!(
        Course,
